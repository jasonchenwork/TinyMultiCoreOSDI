...
static struct DirEntry* get_root_directory(void) {
  ...
  p = get_fs_bpb();
  offset =
      (p->reserved_sector_count + (uint32_t)p->fat_count * p->sectors_per_fat) *
      p->bytes_per_sector;

  return (struct DirEntry*)((uint8_t*)p + offset);
}

static bool is_file_name_equal(struct DirEntry* dir_entry, char* name,
                               char* ext) {
  bool status = false;
...
static uint32_t search_file(char* path) {
  ...
  if (status == true) {
    ...
    for (uint32_t i = 0; i < root_entry_count; i++) {
      if (dir_entry[i].name[0] == ENTRY_EMPTY ||
          dir_entry[i].name[0] == ENTRY_DELETED)
        continue;
...
static uint32_t read_raw_data(uint32_t cluster_index, char* buffer,
  ...
  if (offset != 0) {
    read_size =
        (offset + size) <= cluster_size ? size : (cluster_size - offset);
...
uint32_t read_file(struct Process* process, int fd, void* buffer,
                   uint32_t size) {
  ...
  read_size = read_raw_data(process->file[fd]->fcb->cluster_index, buffer,
                            position, size);
