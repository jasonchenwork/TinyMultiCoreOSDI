...
void init_timer(void) {
  ...
  out_word(CNTP_EL0 + curcore * 4, (1 << 1));
...
volatile uint64_t core_ticks[4] = {0, 0, 0, 0};
  ...
  switch (tf->trapno) {
    case 1:
      if ((tf->spsr & 0xf) == 0) {
        ...
        printk("curcore %d sync error at %x: %x\r\n", curcore, tf->elr,
               tf->esr);
        while (1) {
        }
      }
    ...
    case 2:
      ...
      irq = in_word(CNTP_STATUS_EL0 + curcore * 4);
      if (irq & (1 << 1)) {
        timer_interrupt_handler();
        core_ticks[curcore]++;  // 每個核加自己的
        schedule = 1;
      } else {
        irq = get_irq_number();
        if (irq & (1 << 19)) {
